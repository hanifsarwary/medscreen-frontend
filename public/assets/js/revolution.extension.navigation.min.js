...
!(function () {
				...
				t.bind('revolution.slide.onafterswap revolution.nextslide.waiting', function () {
					...
					t.find('.tp-bullet').each(function () {
						...
						n.find('.tp-thumb, .tp-tab').each(function () {
							...
							t.data('liref') === e
								? (t.addClass('selected'),
								  t.hasClass('tp-tab') ? i(n.find('.tp-tabs'), v) : i(n.find('.tp-thumbs'), g))
	...
	var i = function (t, e) {
			...
			p > 2 &&
				...
				t.hasClass('dragged') ||
					('vertical' === o
						? s.data(
								'tmmove',
								punchgs.TweenLite.to(s, 0.5, { top: g + 'px', ease: punchgs.Power3.easeInOut })
						  )
						: s.data(
								'tmmove',
								punchgs.TweenLite.to(s, 0.5, { left: g + 'px', ease: punchgs.Power3.easeInOut })
						  ),
					s.data('offset', g));
		},
		...
		r = function (e, i) {
			'on' === i.navigation.keyboardNavigation &&
				jQuery(document).keydown(function (a) {
					(('horizontal' == i.navigation.keyboard_direction && 39 == a.keyCode) ||
						('vertical' == i.navigation.keyboard_direction && 40 == a.keyCode)) &&
						((i.sc_indicator = 'arrow'), (i.sc_indicator_dir = 0), t.callingNewSlide(i, e, 1)),
						(('horizontal' == i.navigation.keyboard_direction && 37 == a.keyCode) ||
							('vertical' == i.navigation.keyboard_direction && 38 == a.keyCode)) &&
							((i.sc_indicator = 'arrow'), (i.sc_indicator_dir = 1), t.callingNewSlide(i, e, -1));
				});
		},
		s = function (e, i) {
				...
				e.on('mousewheel DOMMouseScroll', function (s) {
					...
					if (-1 == l) {
						if (o.pixelY < a) {
							...
							return (i.sc_indicator = 'arrow'), (i.sc_indicator_dir = 1), t.callingNewSlide(i, e, 1), !1;
					} else if (!p) return !1;
					s.preventDefault();
				});
			}
		...
		o = function (t, i, a) {
			return (
				(t = e
					? jQuery(a.target).closest('.' + t).length || jQuery(a.srcElement).closest('.' + t).length
					: jQuery(a.toElement).closest('.' + t).length || jQuery(a.originalTarget).closest('.' + t).length),
				t === !0 || 1 === t ? 1 : 0
			);
		},
		d = function (i, a, n) {
			...
			var s = (e || 'Firefox' === t.get_browser(), i),
				d =
					'vertical' === a.navigation.thumbnails.direction || 'vertical' === a.navigation.tabs.direction
						? 'none'
						: 'vertical',
									...
									if (
										...
										var g =
											'center' === r.horizontal_align
												? (r.wrapwidth / 2 - r.slide_width / 2 - r.slide_offset) / r.slide_width
												: (0 - r.slide_offset) / r.slide_width;
										('none' !== r.overpull && 0 !== r.overpull) || !(0 > g || g > a.slideamount - 1)
											? g >= 0 &&
											  g <= a.slideamount - 1 &&
											  ((g >= 0 && d > r.overpull) ||
							...
							if (
								...
								if ('end' == n && !f) {
									if (
										...
										return (a.sc_indicator_dir = 0), t.callingNewSlide(a, a.c, 1), !1;
									if (('horizontal' == l && 'right' == s) || ('vertical' == l && 'down' == s))
										return (a.sc_indicator_dir = 1), t.callingNewSlide(a, a.c, -1), !1;
								}
								return (
									(a.dragStartedOverSlider = !1),
									(a.dragStartedOverThumbs = !1),
										...
										(k =
											k > 0
												? 'horizontal' === T
													...
													: k - y.height() * (((k / y.height()) * k) / y.height())
												: k);
									var O =
										'vertical' === T ? 0 - (y.height() - x.height()) : 0 - (y.width() - x.width());
									(k =
										O > k
											? 'horizontal' === T
												? k + (((y.width() * (k - O)) / y.width()) * (k - O)) / y.width()
												: k + (((y.height() * (k - O)) / y.height()) * (k - O)) / y.height()
											: k),
										'vertical' === T
		...
		l = function (t) {
			(t.hide_delay = jQuery.isNumeric(parseInt(t.hide_delay, 0)) ? t.hide_delay / 1e3 : 0.2),
				(t.hide_delay_mobile = jQuery.isNumeric(parseInt(t.hide_delay_mobile, 0))
		...
		u = function (t, i) {
			...
			p(i.navigation.arrows) &&
				punchgs.TweenLite.delayedCall(
					e ? i.navigation.arrows.hide_delay_mobile : i.navigation.arrows.hide_delay,
					c,
					[a.find('.tparrows'), i.navigation.arrows, 'hide']
				),
				p(i.navigation.bullets) &&
					punchgs.TweenLite.delayedCall(
						e ? i.navigation.bullets.hide_delay_mobile : i.navigation.bullets.hide_delay,
						c,
						[a.find('.tp-bullets'), i.navigation.bullets, 'hide']
					),
				p(i.navigation.thumbnails) &&
					punchgs.TweenLite.delayedCall(
						e ? i.navigation.thumbnails.hide_delay_mobile : i.navigation.thumbnails.hide_delay,
						c,
						[a.find('.tp-thumbs'), i.navigation.thumbnails, 'hide']
					),
				p(i.navigation.tabs) &&
					punchgs.TweenLite.delayedCall(
						e ? i.navigation.tabs.hide_delay_mobile : i.navigation.tabs.hide_delay,
						c,
		...
		f = function (t, e, i) {
			(e.style = void 0 === e.style ? '' : e.style),
				...
				1 === t.find('.tp-leftarrow.tparrows').length &&
					t.append(
						'<div class="tp-leftarrow tparrows ' + e.style + ' ' + e.left.style + '">' + e.tmp + '</div>'
					),
				1 === t.find('.tp-rightarrow.tparrows').length &&
					t.append(
			...
			if (
				...
				'outer-left' !== e.position || ('fullwidth' != e.sliderLayout && 'fullscreen' != e.sliderLayout)
					? 'outer-right' !== e.position ||
					  ...
					  punchgs.TweenLite.set(t, { right: 0 - t.outerWidth() + 'px', x: e.h_offset + 'px' })
				...
				s > a && 'outer-left' !== e.position && 'outer-right' !== e.position
					? (punchgs.TweenLite.set(t, { left: '0px', x: 0, maxWidth: a - 2 * r + 'px' }),
					  punchgs.TweenLite.set(d, { maxWidth: a - 2 * r + 'px' }))
					: (punchgs.TweenLite.set(t, { maxWidth: s + 'px' }),
					  punchgs.TweenLite.set(d, { maxWidth: s + 'px' })),
					o + 2 * r > n && 'outer-bottom' !== e.position && 'outer-top' !== e.position
						? (punchgs.TweenLite.set(t, { top: '0px', y: 0, maxHeight: l + h + (n - 2 * r) + 'px' }),
						  punchgs.TweenLite.set(d, { maxHeight: l + h + (n - 2 * r) + 'px' }))
					...
					e.span === !0 && 'vertical' === e.direction
						...
						: e.span === !0 &&
						  ...
						  (punchgs.TweenLite.set(t, { maxWidth: '100%', width: a - 2 * r + 'px', left: 0, x: 0 }),
						  v(d, e));
			}
		},
		b = function (t, e, i, a) {
			...
			'vertical' === e.direction
				? (o.css({ top: (d - 1) * h + 'px', left: '0px' }),
				  n.css({ height: (d - 1) * h + o.outerHeight(), width: o.outerWidth() }))
				: (o.css({ left: (d - 1) * l + 'px', top: '0px' }),
				  n.css({ width: (d - 1) * l + o.outerWidth(), height: o.outerHeight() })),
		...
		_ = function (t, e, i, a, n) {
			...
			if (
				((e.visibleAmount = e.visibleAmount > n.slideamount ? n.slideamount : e.visibleAmount),
			...
			x.find(l).css({ backgroundImage: 'url(' + n.thumbs[i.index()].src + ')' }),
				'vertical' === e.direction
					? (x.css({ top: (y - 1) * L + 'px', left: '0px' }),
					  g.css({ height: (y - 1) * L + x.outerHeight(), width: x.outerWidth() }))
					: (x.css({ left: (y - 1) * T + 'px', top: '0px' }),
					  g.css({ width: (y - 1) * T + x.outerWidth(), height: x.outerHeight() })),
				c.data('maxw', v),
				c.data('maxh', b),
				c.data('wr_padding', e.wrapper_padding);
			var j = 'outer-top' === e.position || 'outer-bottom' === e.position ? 'relative' : 'absolute',
				C =
					('outer-top' !== e.position && 'outer-bottom' !== e.position) || 'center' !== e.h_align
						? '0'
						: 'auto';
