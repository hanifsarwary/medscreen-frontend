...
!(function (e) {
	...
	jQuery.extend(!0, a, {
		resetVideo: function (e, d) {
			switch (e.data('videotype')) {
				...
				case 'html5':
					...
					('mute' == e.data('volume') || a.lastToggleState(e.data('videomutetoggledby'))) && (l.muted = !0);
			}
		},
			...
			switch ((clearTimeout(e.data('videoplaywait')), e.data('videotype'))) {
				case 'youtube':
					if (0 == e.find('iframe').length) e.append(e.data('videomarkup')), r(e, o, !0);
						...
						1 == e.data('nextslideatend-triggered') && ((s = -1), e.data('nextslideatend-triggered', 0)),
		...
		checkVideoApis: function (e, t, a) {
			...
			if (
				((void 0 != e.data('ytid') ||
					(e.find('iframe').length > 0 &&
						e.find('iframe').attr('src').toLowerCase().indexOf('youtube') > 0)) &&
			...
			if (
				((void 0 != e.data('vimeoid') ||
					(e.find('iframe').length > 0 && e.find('iframe').attr('src').toLowerCase().indexOf('vimeo') > 0)) &&
					(t.vimeoapineeded = !0),
				(void 0 != e.data('vimeoid') ||
					(e.find('iframe').length > 0 && e.find('iframe').attr('src').toLowerCase().indexOf('vimeo') > 0)) &&
					0 == a.addedvim)
		...
		manageVideoLayer: function (e, o, s, l) {
			...
			switch ((e.data('videotype', T), k)) {
				case 'html5':
					...
					void 0 != f &&
						...
						void 0 != g && (x = x + '<source src="' + g + '" type="video/mp4" />'),
						void 0 != y && (x = x + '<source src="' + y + '" type="video/ogg" />'),
						(x += '</video>');
					var L = '';
					('true' === m || m === !0) &&
						(L =
						...
						e.append(x),
						((i && 1 == e.data('disablevideoonmobile')) || a.isIE(8)) && e.find('video').remove(),
				...
				case 'youtube':
					(b = 'http'),
						...
						'none' == h &&
							...
							-1 == p.toLowerCase().indexOf('controls') && (p += '&controls=0'));
					...
					I.length > 1
						? ((C = I[0] + 'origin=' + b + '://'),
						  self.location.href.match(/www/gi) && !I[1].match(/www/gi) && (C += 'www.'),
						  (C += I[1]))
						: (C = p);
			...
			'none' != e.data('dottedoverlay') &&
				...
				e.append('<div class="tp-dottedoverlay ' + e.data('dottedoverlay') + '"></div>'),
	...
	var d = function (e, t, a) {
		...
		r = function (e, d, r) {
			...
			if (((v = 'loop' == v || 'loopandnoslidestop' == v), 1 == e.data('forcecover'))) {
				e.removeClass('fullscreenvideo').addClass('coverscreenvideo');
				var c = e.data('aspectratio');
				void 0 != c && c.split(':').length > 1 && a.prepareCoveredVideo(c, d, e);
			}
			if (1 == e.data('bgvideo')) {
				var c = e.data('aspectratio');
				void 0 != c && c.split(':').length > 1 && a.prepareCoveredVideo(c, d, e);
			}
			if ((n.attr('id', p), r && e.data('startvideonow', !0), 1 !== e.data('videolistenerexist')))
				switch (e.data('videotype')) {
					case 'youtube':
									...
									if (e.data == YT.PlayerState.PLAYING)
										punchgs.TweenLite.to(p.find('.tp-videoposter'), 0.3, {
											...
											a.lastToggleState(p.data('videomutetoggledby'))
												? c.mute()
												: (c.unMute(), c.setVolume(parseInt(p.data('volume'), 0) || 75)),
											(d.videoplaying = !0),
											s(p, d),
											u ? d.c.trigger('stoptimer') : (d.videoplaying = !1),
											d.c.trigger('revolution.slide.onvideoplay', o(c, 'youtube', p.data())),
										...
										(0 == e.data || 2 == e.data) &&
											...
											-1 != e.data &&
												...
												d.c.trigger('revolution.slide.onvideostop', o(c, 'youtube', p.data())),
												(void 0 == d.currentLayerVideoIsPlaying ||
													d.currentLayerVideoIsPlaying.attr('id') == p.attr('id')) &&
													a.unToggleState(p.data('videotoggledby'))),
											0 == e.data && 1 == p.data('nextslideatend')
												? (p.data('nextslideatend-triggered', 1), d.c.revnext(), l(p, d))
												: (l(p, d),
												  (d.videoplaying = !1),
												  n.trigger('starttimer'),
												  d.c.trigger(
						...
						break;
					case 'vimeo':
						for (var f, y = n.attr('src'), m = {}, h = y, b = /([^&=]+)=([^&]*)/g; (f = b.exec(h)); )
							m[decodeURIComponent(f[1])] = decodeURIComponent(f[2]);
						y = void 0 != m.player_id ? y.replace(m.player_id, p) : y + '&player_id=' + p;
						try {
							...
							if (
								...
								T.addEvent('play', function (t) {
									e.data('nextslidecalled', 0),
										...
										d.c.trigger('revolution.slide.onvideoplay', o(T, 'vimeo', e.data())),
										(d.videoplaying = !0),
										s(e, d),
										u ? d.c.trigger('stoptimer') : (d.videoplaying = !1),
										'mute' == e.data('volume') || a.lastToggleState(e.data('videomutetoggledby'))
											? T.api('setVolume', '0')
											: T.api('setVolume', parseInt(e.data('volume'), 0) / 100 || 0.75),
								...
								T.addEvent('finish', function (t) {
									l(e, d),
										...
										d.c.trigger('revolution.slide.onvideostop', o(T, 'vimeo', e.data())),
										1 == e.data('nextslideatend') &&
											(e.data('nextslideatend-triggered', 1), d.c.revnext()),
										(void 0 == d.currentLayerVideoIsPlaying ||
											d.currentLayerVideoIsPlaying.attr('id') == e.attr('id')) &&
				...
				switch (e.data('videotype')) {
					case 'youtube':
						r && (e.data('player').playVideo(), -1 != k && e.data('player').seekTo());
						break;
					case 'vimeo':
						if (r) {
		...
		n = function (e, r, n) {
			...
			void 0 != y &&
				...
				void 0 != m &&
					d(m, 'click', function () {
						0 == v.muted
							? ((v.muted = !0), (m.innerHTML = 'Unmute'))
							: ((v.muted = !1), (m.innerHTML = 'Mute'));
					}),
				void 0 != h &&
					h &&
					d(h, 'click', function () {
				...
				d(v, 'timeupdate', function () {
					...
					if (
						(void 0 != b && (b.value = a),
						0 != i && -1 != i && Math.abs(i - d) <= 0.3 && i > d && 1 != e.data('nextslidecalled'))
					)
						if (c) {
							v.play();
							var o = t(e.data('videostartat'));
				...
				d(v, 'play', function () {
					e.data('nextslidecalled', 0),
						...
						g
							? ((r.videoplaying = !0),
							  ...
							  r.c.trigger('revolution.slide.onvideoplay', o(v, 'html5', e.data())))
							: ((r.videoplaying = !1),
							  r.c.trigger('starttimer'),
							  r.c.trigger('revolution.slide.onvideostop', o(v, 'html5', e.data()))),
						punchgs.TweenLite.to(e.find('.tp-videoposter'), 0.3, {
							autoAlpha: 0,
							force3D: 'auto',
							ease: punchgs.Power3.easeInOut,
					...
					void 0 != t && (t.innerHTML = 'Pause'),
						void 0 != i && v.muted && (i.innerHTML = 'Unmute'),
						a.toggleState(e.data('videotoggledby'));
				}),
				d(v, 'pause', function () {
					e.find('.tp-videoposter').length > 0 &&
						...
						punchgs.TweenLite.to(e.find('video'), 0.3, { autoAlpha: 0, ease: punchgs.Power3.easeInOut })),
