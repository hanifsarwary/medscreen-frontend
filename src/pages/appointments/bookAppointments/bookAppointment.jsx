...
class BookAppointment extends Component {
  ...
  handleSubmit = () => {
    ...
    const payload = {
      panels: selected_options.map((item) => {
        return item.value;
      }),
      appointment_date,
      ...
      case 1:
        return (
          <div class="col-sm-12">
            <div class="forms-field fade-apply appointment-form-margin">
              ...
              <div class="select-category">
                {this.props.tests.length > 0 ? (
                  <NestedCheckBox
                    handleSubCheckBox={this.handleSubCheckBox}
            ...
            <button className="btn btn-primary pull-right" onClick={this.nextStep}>
              Next
            </button>
          </div>
        );
        ...
        return (
          <>
            {categories.length > 0 ? (
                    ...
                    <div className="row" key={i}>
                      ...
                      {item.panel_options.length > 0 ? (
                        <SelecCategory
                          ...
                          tests={item}
                          handleTestOptionValue={this.handleTestOptionValue}
                        />
                      ) : (
                        ''
                      )}
              ...
              <div className="user-message fade-apply set-margin-bottom">Please Select Category</div>
            )}
            <button className="btn btn-primary pull-left" onClick={this.clearObj}>
              Previous
            </button>
            <button
              className="btn btn-primary pull-right"
              disabled={categories.length > 0 ? false : true}
              onClick={this.getTestBill}
            >
              Next
            </button>
          </>
      ...
      case 3:
        return (
          <div>
            ...
            <button
              className="btn btn-primary pull-right mr-0"
              disabled={categories.length > 0 ? false : true}
            ...
            {this.state.selected_options.length > 0 ? (
              <div class="col-sm-12">
                <div class="forms-field fade-apply set-margin-bottom">
                  ...
                  <input id="date" type="date" name="appointment_date" onChange={this.getTimeSlots} min={new Date()} />
                </div>
              </div>
            ) : (
              <div className="user-message fade-apply set-margin-bottom">Please Select Panel</div>
            )}
            {appointment_date ? (
              <div class="col-sm-12">
                {time_slots.length > 0 ? (
                  ...
                  <div className="user-message fade-apply set-margin-bottom">No Time Slot Avaiable</div>
                )}
              </div>
            ) : (
              ''
            )}
            <button className="btn btn-primary pull-left set-margin-top" onClick={this.prevStep}>
              Previous
            </button>
            <button
              className="btn btn-primary pull-right set-margin-top"
